...
| `warehouses` | Multi-location stock + transfers + per-warehouse RBAC | [Warehouses](../operations/warehouses.md) |
...
| `hr_contracts` | Formal employment contracts | [HR Contracts](../people/hr-contracts.md) |
| `hr_activities` | Per-employee personal calendar with reminders | [HR Activities](../people/hr-activities.md) |
...
| `roles` | RBAC role definitions + per-action grants | [RBAC](../foundation/rbac.md) |
...
| Multi-warehouse access | Row-level access on warehouses | [Warehouse access](../foundation/warehouse-access.md) |
...
| People | 8 (`hr`, `hr_contracts`, `hr_activities`, `recruitment`, `approvals`, `planning`, `announcements`, `notifications`) |
...
|---|---|---|
| `warehouses` | Phase 1 of multi-warehouse rollout | Multi-location stock + row-level access |
| `accounting` | Phase 1 of accounting GL | Double-entry general ledger |
| `hr_contracts` | HR maturity work | First-class contract documents |
| `hr_activities` | HR maturity work | Per-employee calendar |
| `announcements` | Internal comms feature | Audience-targeted broadcasts |

All five are RBAC-enabled and show in the Roles & Permissions matrix.
