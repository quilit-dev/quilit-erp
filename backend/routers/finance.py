...
def create_expense(
    ...
    cur = db.execute(
        ...
        (data.project_id, data.category, data.description,
         data.amount, data.date or datetime.utcnow().strftime("%Y-%m-%d"), now, "Recorded"),
    )
...
def update_expense(
    ...
    db.execute(
        "UPDATE expenses SET project_id=?, category=?, description=?, amount=?, date=? WHERE id=?",
        (new_project_id, data.category, data.description,
         new_amount, data.date or exp["date"], expense_id),
    )
...
def reconciliation(
    ...
    # ── Tax multiplier ──────────────────────────────────────────────────────
    te = db.execute("SELECT value FROM settings WHERE key='tax_enabled'").fetchone()
    tr = db.execute("SELECT value FROM settings WHERE key='default_tax_rate'").fetchone()
    tax_enabled = te and te["value"] == "1"
    try:
        tax_rate = float(tr["value"]) if tr else 0.0
    except Exception:
        tax_rate = 0.0
    mult = 1.0 + tax_rate / 100.0 if tax_enabled else 1.0

    # ── 1. Invoice amount vs line-items sum (VAT recalculation) ────────────
    rows = db.execute("""
        ...
        GROUP BY i.id
        HAVING item_count > 0
           AND ABS(i.amount - ROUND(items_subtotal * ?, 4)) > 0.05
    """, (mult,)).fetchall()
    for r in rows:
        expected = round(float(r["items_subtotal"]) * mult, 2)
        issues.append({
            "type": "vat_mismatch", "severity": "error",
            "invoice_number": r["invoice_number"], "invoice_id": r["id"],
            "message": (
                f"{r['invoice_number']}: stored ${r['amount']:.2f} ≠ "
                f"line items total ${expected:.2f}"
                + (f" (incl. {tax_rate}% VAT)" if tax_enabled else "")
            ),
        })
