...
**Phase 4 is complete** — Redis-backed caching (`CACHE=redis`) and background jobs (`JOBS=rq`) are available behind flags that default to off, so nothing changes for desktop/single-tenant installs. The infrastructure (`cache.py`, `jobs.py`, `worker.py`) is tenant-aware and wired into representative paths (settings caching; the attachment backfill as a job); remaining fan-outs migrate onto it incrementally. **Run a worker:** `JOBS=rq REDIS_URL=… python backend/worker.py`. **Next: Phase 5 — Docker + Hetzner deployment** (§10).
