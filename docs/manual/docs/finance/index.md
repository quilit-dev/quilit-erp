...
|---|---|---|
| **Cash-basis Finance** | `invoice_payments`, `expenses` rows | "What's in the bank?" "What did we spend this month?" |
| **Double-entry GL** | `journal_entries` + `journal_entry_lines` | "Show me the Trial Balance." "Are we profitable accrual-basis?" |
...
| F-6 — Payroll mis-posted LBP face value | Per-line `salary_currency` snapshot + spot-rate conversion |
| F-8 — No FX revaluation | `POST /api/accounting/fx-revaluation` marks LBP cash to market |
