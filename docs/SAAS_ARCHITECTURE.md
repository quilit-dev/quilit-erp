...
**Phase 3 is complete** — all three file stores (generic attachments, HR employee files, recruitment applicant files) can live in S3/R2 instead of DB BLOBs (`STORAGE=s3`), with a backfill for existing attachments; default `STORAGE=db` keeps current behavior. **Next: Phase 4 — Redis (cache + background jobs)** (§9).
