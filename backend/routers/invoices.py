...
class PaymentCreate(BaseModel):
    ...
    # Which bank account a transfer, cheque or card settlement landed in. The
    # column has existed since bank accounts did; nothing ever filled it, so
    # every bank movement piled into one line and no balance could be
    ...
    for pay in db.execute(
        ...
        accounting.post_entry(
            ...
            lines=accounting.payment_lines(
                db, invoice_id,
                # Re-posted where the original went, bank account and
                # all, or an unvoid quietly moves money between accounts.
                cash_code=accounting.money_account_for(
                    db, method=pay["method"], currency=pay["paid_currency"],
                    bank_account_id=_col(pay, "bank_account_id")),
...
def add_payment(
    ...
    pay_cur = db.execute(
        ...
        " bank_account_id, txn_amount, fx_difference) "
        "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)",
        ...
        (invoice_id, s["obligation_base"], data.method, data.note, _now(),
         ...
         data.bank_account_id, s["txn_settled"], s["fx_difference"]),
    )
    ...
    accounting.post_entry(
        ...
        lines=accounting.payment_lines(
            db, invoice_id,
            # Where the money actually went, not where cash goes. A
            # transfer used to debit cash-on-hand, so the till was overstated
            # by every payment that never touched it and the bank account
            # showed nothing at all.
            cash_code=accounting.money_account_for(
                db, method=data.method, currency=currency,
