...
def void_expense(
    ...
    branch_access.assert_can_view_branch(user, db, exp["branch_id"])
    if exp["voided_at"]:
    ...
    # A recurring bill covering several months is ONE payment split across the
    # months it buys. Voiding a single month of it is not a real operation: the
    # money moved once, and reversing only that month's share would leave the
    # prepayment standing on the balance sheet with nothing left to release it.
    # So the occurrence is the unit — void any row and the whole payment is
    # undone, cash included. (A partial refund is a different transaction.)
    #
    # Rows generated before this column existed have it NULL and fall through to
    # the single-row path, exactly as they did before.
    group = exp["recurring_occurrence_id"] if "recurring_occurrence_id" in exp.keys() else None
    siblings = []
    if group:
        # Archived rows are INCLUDED. Archiving only hides a row from the lists
        # and totals; its ledger entry stays live, so leaving it out would
        # reverse the payment while one month's cost stayed on the books.
        # Already-voided rows are excluded — they are reversed already.
        siblings = db.execute(
            "SELECT * FROM expenses WHERE recurring_occurrence_id = ? "
            " AND voided_at IS NULL ORDER BY date",
            (group,)).fetchall()
    targets = siblings if len(siblings) > 1 else [exp]

    # Every month being reversed must be in an open period, not just the one
    # that was clicked — a later month can sit in a closed one, and discovering
    # that halfway through would leave the occurrence half undone.
    for row in targets:
        _check_period_locked(db, row["date"])
    ...
    for row in targets:
        ...
        db.execute(
            ...
            (data.reason or None, row["id"]),
        )
        # Reverse the ledger entry posted when the expense was recorded.
        accounting.reverse_source(db, "expense", row["id"],
                                  memo=f"Reversal — voided expense ({row['category']})",
                                  created_by=user["id"])

    # And the payment that created them: DR Prepaid / CR Cash reversed, so the
    # money comes back and the prepayment returns to nil. Keyed on the first row
    # of the occurrence (see recurring._post_occurrence). Returns None for a
    # single-month expense, which never had a separate payment entry.
    if len(targets) > 1:
        accounting.reverse_source(
            db, "prepaid_payment", group,
            memo=f"Reversal — voided recurring payment ({exp['category']})",
            created_by=user["id"])

    total = round(sum(float(r["amount"]) for r in targets), 2)
    log_action(db, user, "void", "expense", expense_id,
               exp["category"], {"amount": total, "reason": data.reason,
                                 "rows": len(targets)})
    db.commit()
    if len(targets) > 1:
        return {"message": f"Voided the whole recurring payment — "
                           f"{len(targets)} monthly entries, ${total:,.2f}",
                "voided_count": len(targets)}
    return {"message": "Expense voided", "voided_count": 1}
...
def archive_expense(
    ...
    if exp["fixed_asset_id"]:
        raise HTTPException(
            400, "This is a depreciation entry generated by Fixed Assets. "
            "Manage it from the Fixed Assets module.")
    # Void first, then archive. Archiving is only a filing action now, which
    # is why the project-cost adjustment that used to live here is gone:
    # `void_expense` already reverses the contribution to `actual_cost`, and
    # doing it again here would subtract the same money twice.
    if not exp["voided_at"]:
        raise HTTPException(
            400,
            "Void this expense before archiving it. An expense that has not "
            "been voided still counts in the figures, and archiving would "
            "only hide it.")
    _check_period_locked(db, exp["date"])
    now = _now()
