...
| **3 — Attachments → object storage** | 🟢 **Core done** | `storage.py`: a `db`/`s3` abstraction (lazy `boto3`; S3 / Cloudflare R2 / MinIO), tenant-scoped keys `<schema>/<entity>/<id>/<uuid>`. Migration 124 adds `attachments.storage_backend` + `storage_key` (additive — no rebuild). The attachments API routes uploads to object storage when `STORAGE=s3`, streams downloads back (RBAC re-checked — no public presigned URLs), cleans up objects on delete, and offers a superadmin `POST /api/attachments/migrate-to-s3` backfill. `test_storage.py` proves the s3 round-trip + backfill via mocked S3 (moto) on **both** backends; `test_attachments.py` remains the `db`-default regression gate. Default `STORAGE=db` = unchanged behavior. **Remaining:** apply the same to `hr_employee_files` / `recruitment_applicant_files`. |
| 4–6 | ⬜ Planned | — |
...
**Phase 3 core is complete** — attachments can live in S3/R2 instead of DB BLOBs (`STORAGE=s3`), with a backfill for existing files; default `STORAGE=db` keeps current behavior. **Next:** extend object storage to the HR/recruitment file tables, then **Phase 4 — Redis (cache + background jobs)** (§9).
