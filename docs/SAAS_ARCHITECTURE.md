...
| **2 — Multi-tenancy (schema-per-tenant)** | 🟢 **Core done** | `tenant_context.py` + `tenancy.py`: `public.tenants` catalog, `provision_tenant()`, and a pure-ASGI `TenantMiddleware` that resolves the tenant (signed JWT `schema` claim → `X-Tenant` header → host subdomain) into a ContextVar; `get_db` pins the connection's `search_path`. Login binds the JWT to the resolved tenant. `test_multitenancy.py` proves isolation (no cross-tenant data; a tenant-A token can't be redirected to B via a forged header). Inert in single-tenant mode (`TENANCY=single`, default) — single-tenant suite unchanged. Dedicated CI job runs it. **Remaining (lifecycle):** platform-operator auth + a provisioning/suspend API + self-service signup. |
| 3–6 | ⬜ Planned | — |
...
**Phase 2 core is complete** — schema-per-tenant isolation works end-to-end (see `tenancy.py`, `test_multitenancy.py`). Isolation is structural: each tenant is a Postgres schema with its own users/roles/permissions/audit (ADR-3); a query in one schema cannot name another's tables, and a session token is cryptographically bound to its tenant. **Next:** the tenant *lifecycle* surface (platform-operator auth via `public.platform_admins`, a provisioning/suspend API, self-service signup), then **Phase 3 — attachments → object storage (R2/S3)** (§8).
