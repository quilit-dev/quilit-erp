...
| **6 — Layering** | ✅ **Done** | The layers exist as seams, not a rewrite (the Phase-6 rule): **API** = the routers (unchanged); **tenant** = `TenantMiddleware`; **data** = `get_db` + `CompatConn` + the new `database.session()` for non-request access (workers/jobs/scripts); **service** = the pure modules `accounting.py` / `costing.py` / `lots.py` / `approval_engine.py`; **infra** = `cache.py` / `jobs.py` / `storage.py`; **workers** = `worker.py` + the RQ registry. Thin extraction this phase: `database.session()` (replaces the ad-hoc `next(get_db())` in the backfill job), tested on both backends. No working module was rewritten. |
...
**Phase 5 is complete** — the whole stack runs from `docker-compose.yml` (Caddy + app + worker + Postgres + Redis + backup), TLS/HSTS via Caddy, nightly `pg_dump` backups, non-root containers, and a hardening checklist. See **[DEPLOYMENT.md](DEPLOYMENT.md)**. The app image was built and smoke-tested against Postgres (bootstrap → 91 tables + admin, health 200). No app-logic change beyond a `/api/health` liveness route.

**Phase 6 is complete** — the architecture is layered through seams rather than a rewrite (see the status table). The only extraction was `database.session()`, a tenant-scoped connection for non-request contexts (workers/jobs/scripts) that mirrors `Depends(get_db)`; the attachment backfill job now uses it instead of an ad-hoc `next(get_db())`. Tested on both backends. Per the Phase-6 rule, no working module was rewritten — further per-module repositories are added only when a module is next touched for a feature.

---

### ✅ Migration complete (Phases 0–6)

All six phases are done. One codebase now serves **both** the desktop / self-hosted product (SQLite, single-tenant, DB storage, synchronous — the defaults, byte-for-byte unchanged) **and** a production cloud SaaS (PostgreSQL, schema-per-tenant, S3/R2 storage, Redis cache + jobs, Dockerized on Hetzner) — selected entirely by the flags in Appendix A. The full test-suite passes on both SQLite and PostgreSQL; no business logic, calculation, RBAC, approval, audit, report, or API contract was changed.
