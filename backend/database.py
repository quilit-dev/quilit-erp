...
def _run_migrations(conn, c):
    ...
    # ── 164: a payment plan against the ACCOUNT ──────────────────────────
    # A customer owing 4,000 who agrees to eight payments of 500 has agreed
    # one thing, not eight schedules pinned to whichever invoices happen to be
    # open today. Decomposing it onto the invoices at the moment it is agreed
    # looks equivalent and is not: a new invoice raised next week is outside
    # it, voiding one tears a hole in it, and nobody can be shown the eight
    # payments they actually agreed to.
    #
    # So the plan lives on the customer. Each payment against it is an
    # ordinary account payment and is allocated oldest-first exactly as any
    # other, which is what keeps every invoice, statement and ledger posting
    # working without learning about plans at all.
    if need("164_client_payment_plans"):
        c.execute("""
        ...
        """)
        # One live plan per customer: two active agreements about the same
        # balance is not something anybody agreed to.
        c.execute("CREATE UNIQUE INDEX IF NOT EXISTS idx_client_plan_active "
                  "ON client_payment_plans(client_id) WHERE status = 'active'")
        c.execute("""
            CREATE TABLE IF NOT EXISTS client_plan_installments (
                id        INTEGER PRIMARY KEY AUTOINCREMENT,
                plan_id   INTEGER NOT NULL REFERENCES client_payment_plans(id)
                          ON DELETE CASCADE,
                seq       INTEGER NOT NULL,
                due_date  TEXT    NOT NULL,
                amount    REAL    NOT NULL
        ...
        """)
        c.execute("CREATE UNIQUE INDEX IF NOT EXISTS idx_client_plan_seq "
                  "ON client_plan_installments(plan_id, seq)")
        c.execute("CREATE INDEX IF NOT EXISTS idx_client_plan_due "
                  "ON client_plan_installments(due_date)")
        done("164_client_payment_plans")

    # Which plan a payment was made against. Nothing is stored about WHICH
    # instalment it settled: that is derived by comparing cumulative paid
    # against cumulative scheduled, exactly as the invoice plans do, so the
    # two can never drift apart.
    add_col("164a_payment_plan_link", "customer_payments", "plan_id",
    ...
    # ── 170: goods sold and paid for that are not on the shelf yet ─────
    # A customer wants five, there are two, the manager says the rest can be
    # got, and the customer pays for five. The two that exist leave stock at
    # the till; the three that do not are a promise, and a promise is not
    # inventory. Writing them as negative stock would make the count a
    # guess, and reserving them would drive availability below zero, which is
    # the same untruth from the other side.
    #
    # So they live here instead: what is owed, to whom, at what price, and how
    # much of it has since been allocated or handed over. `inventory.quantity`
    # is untouched by any of it.
    if need("170_sale_commitments"):
        c.execute("""
            CREATE TABLE IF NOT EXISTS sale_commitments (
                ...
                -- Frozen at the till. What the customer paid is what they paid,
                -- whatever the price does between now and the goods arriving.
                unit_price         REAL    NOT NULL DEFAULT 0,
                unit_tax           REAL    NOT NULL DEFAULT 0,
        ...
        """)
        # Allocation walks the open rows for one item, oldest first.
        c.execute("CREATE INDEX IF NOT EXISTS idx_commitments_item "
                  "ON sale_commitments(inventory_id, status)")
        # "What is this customer still waiting for?" is asked from their record.
        c.execute("CREATE INDEX IF NOT EXISTS idx_commitments_client "
                  "ON sale_commitments(client_id, status)")
        c.execute("CREATE INDEX IF NOT EXISTS idx_commitments_invoice "
                  "ON sale_commitments(invoice_id)")
        # Each hand-over is its own event: two of three can arrive on Tuesday
        # and the last on Friday, and each posts its own entry. Keyed on this
        # row rather than the commitment so the second delivery is not mistaken
        # for a re-run of the first and silently dropped.
        c.execute("""
            CREATE TABLE IF NOT EXISTS commitment_deliveries (
                id            INTEGER PRIMARY KEY AUTOINCREMENT,
                commitment_id INTEGER NOT NULL REFERENCES sale_commitments(id) ON DELETE CASCADE,
                quantity      REAL    NOT NULL,
                unit_cost     REAL    NOT NULL DEFAULT 0,
                warehouse_id  INTEGER REFERENCES warehouses(id),
                note          TEXT,
        ...
        """)
        c.execute("CREATE INDEX IF NOT EXISTS idx_deliveries_commitment "
                  "ON commitment_deliveries(commitment_id)")
        done("170_sale_commitments")
    ...
    # source id at all, so the only trace of WHY the books moved was a memo
    # string. That cannot be reconciled, reversed by reference, or explained
    # at year end.
    #
    # A run now records what was counted, what the books said, at which rate,
    # and which entry carried it — so every unrealised difference points back
    # at the count that produced it.
    if need("163_fx_revaluation_runs"):
        c.execute("""
        ...
        """)
        c.execute("CREATE INDEX IF NOT EXISTS idx_fx_runs_asof "
                  "ON fx_revaluation_runs(as_of)")
        done("163_fx_revaluation_runs")

    # The accountant's own mark. Every difference in the system is posted the
    # moment it arises; what an accountant adds at period end is having LOOKED
    # at it. Kept apart from the transactions so signing one off never touches
    # the record that produced it.
    if need("163_fx_reconciliations"):
        c.execute("""
            CREATE TABLE IF NOT EXISTS fx_reconciliations (
                id             INTEGER PRIMARY KEY AUTOINCREMENT,
                kind           TEXT NOT NULL,        -- realized | unrealized
                ref_id         INTEGER NOT NULL,     -- payment id, or run id
        ...
        """)
        c.execute("CREATE UNIQUE INDEX IF NOT EXISTS idx_fx_recon_ref "
                  "ON fx_reconciliations(kind, ref_id)")
        done("163_fx_reconciliations")
    ...
    # ── 175: a service job is attended by PEOPLE, not by a login ─────────
    # `service_jobs.assigned_to` points at users(id), so only somebody with an
    # account could be recorded as having done the work. Field technicians
    # normally have no account, and the picker offered every user in the system
    # with no way to tell a technician from an accountant. `hr_employees`
    # already holds the right people — including a "Technician" job title — and
    # a job commonly takes more than one of them, which one column cannot say.
    #
    # `assigned_to` is deliberately NOT dropped, against the precedent set by
    # 172e. Nothing can be carried across: employees are linked to logins by
    # hr_employees.user_id and on the tenant this was checked against NONE of
    # them had one, so a backfill would preserve nothing while destroying the
    # assignments that do exist. It stays as read-only history — never written
    # again, still readable on jobs raised before this — because losing the
    # record of who did the work is worse than a column that no longer moves.
    if need("175_service_job_technicians"):
        c.execute("""
            CREATE TABLE IF NOT EXISTS service_job_technicians (
                id          INTEGER PRIMARY KEY AUTOINCREMENT,
                job_id      INTEGER NOT NULL REFERENCES service_jobs(id) ON DELETE CASCADE,
                employee_id INTEGER NOT NULL REFERENCES hr_employees(id),
                created_at  TEXT    NOT NULL,
                -- The count IS the product here. Without this, one person added
                -- to a job twice inflates their month and nothing fails.
                UNIQUE (job_id, employee_id)
            )""")
        c.execute("CREATE INDEX IF NOT EXISTS idx_sjt_job "
                  "ON service_job_technicians(job_id)")
        c.execute("CREATE INDEX IF NOT EXISTS idx_sjt_employee "
...
def _ensure_pg_post_baseline(raw):
    ...
    with raw.cursor() as cur:
        # Created first: several statements below write roles into it, and
        # they had drifted above the line that made it. On a database that
        # already had the table nothing failed, so a fresh one was the only
        # place it showed — and fresh databases are exactly what a new
        # deployment is.
        cur.execute(
            "CREATE TABLE IF NOT EXISTS account_roles "
        ...
        for _col, _type in (
            ...
            cur.execute("ALTER TABLE fixed_assets ADD COLUMN IF NOT EXISTS "
                        + _col + " " + _type)
        # 170_sale_commitments — what a customer has paid for and not yet
        # received. Created after every other table above: it points at
        # invoices, invoice_items, inventory, clients and warehouses, and a
        # fresh database builds this file top to bottom.
        cur.execute("""
            CREATE TABLE IF NOT EXISTS sale_commitments (
                id                 INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
        ...
        """)
        cur.execute("CREATE INDEX IF NOT EXISTS idx_commitments_item "
                    "ON sale_commitments(inventory_id, status)")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_commitments_client "
                    "ON sale_commitments(client_id, status)")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_commitments_invoice "
                    "ON sale_commitments(invoice_id)")
        ...
        """)
        cur.execute("CREATE INDEX IF NOT EXISTS idx_deliveries_commitment "
                    "ON commitment_deliveries(commitment_id)")
        ...
        # 164: a payment plan against the account rather than the invoices.
        cur.execute("""
            CREATE TABLE IF NOT EXISTS client_payment_plans (
                id          INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                client_id   INTEGER NOT NULL REFERENCES clients(id),
        ...
        """)
        cur.execute("CREATE UNIQUE INDEX IF NOT EXISTS idx_client_plan_active "
                    "ON client_payment_plans(client_id) WHERE status = 'active'")
        cur.execute("""
            CREATE TABLE IF NOT EXISTS client_plan_installments (
                id        INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
        ...
        """)
        cur.execute("CREATE UNIQUE INDEX IF NOT EXISTS idx_client_plan_seq "
                    "ON client_plan_installments(plan_id, seq)")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_client_plan_due "
                    "ON client_plan_installments(due_date)")
        ...
        """)
        cur.execute("CREATE INDEX IF NOT EXISTS idx_fx_runs_asof "
                    "ON fx_revaluation_runs(as_of)")
        cur.execute("""
            CREATE TABLE IF NOT EXISTS fx_reconciliations (
                id             INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
        ...
        """)
        cur.execute("CREATE UNIQUE INDEX IF NOT EXISTS idx_fx_recon_ref "
                    "ON fx_reconciliations(kind, ref_id)")
        ...
        # revalued without unpicking it from the dollars. Placed AFTER the
        # role table is created: the loop above seeds it on a fresh database,
        # and this reaches a tenant whose role migration has already run.
        cur.execute(
            "INSERT INTO chart_of_accounts "
            "(code, name, type, subtype, normal_balance, is_system, is_active, created_at) "
            "VALUES ('1020','Cash — EUR','Asset','Current Asset','debit',1,1,now()::text) "
        ...
        # 175: a service job is attended by people, not by a login. Several of
        # them, from hr_employees. No backfill — see the SQLite chain for why
        # service_jobs.assigned_to is kept rather than dropped.
        cur.execute("""CREATE TABLE IF NOT EXISTS service_job_technicians (
            id          INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
            job_id      INTEGER NOT NULL,
            employee_id INTEGER NOT NULL,
            created_at  TEXT NOT NULL,
            UNIQUE (job_id, employee_id))""")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_sjt_job "
                    "ON service_job_technicians(job_id)")
